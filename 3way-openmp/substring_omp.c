...
void getSubStr(char* x, char* y, int m, int n, char** out)
{
  ...
  uint16_t len = 0;   /* longest common substring length */
  uint16_t max_len;   /* max length of substring */
  uint16_t row, col;  /* index of cell holding max value */
  
  /* allocate space for common suffixes */
  ...
  /* fill com_suff[m+1][n+1] from bottom up */
  for (i = 0; i <= m; i++)
  {
    for (j = 0; j <= n; j++)
    {
